...
describe("Modern File Finding Tool", () => {
  ...
  it("should handle recursive search", async () => {
    ...
    expect(parsed.files.some((f: string) => f.endsWith("nested.js"))).toBe(
      true
    );
    expect(parsed.totalFiles).toBe(2);
  ...
  it("should handle maxResults limit", async () => {
    ...
    expect(parsed.files.some((f: string) => f.includes("test1.txt"))).toBe(
      true
    );
    expect(parsed.files.some((f: string) => f.includes("test5.txt"))).toBe(
      true
    );
