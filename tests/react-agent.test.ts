...
describe("React Agent and Tools Integration", () => {
  ...
  describe("FileListTool", () => {
    it("should list directory contents", async () => {
      ...
      expect(parsed.entries.some((e: any) => e.name === "test1.txt")).toBe(
        true
      );
      expect(parsed.entries.some((e: any) => e.name === "test2.txt")).toBe(
        true
      );
  ...
  describe("FileReadTool", () => {
    ...
    it("should handle different file types", async () => {
      ...
      const jsResult = await fileReadTool.invoke(
        { filePath: jsFile },
        { context: testContext }
      );
      const jsonResult = await fileReadTool.invoke(
        { filePath: jsonFile },
        { context: testContext }
      );
      const txtResult = await fileReadTool.invoke(
        { filePath: txtFile },
        { context: testContext }
      );
  ...
  describe("FileFindTool", () => {
    it("should find files by pattern", async () => {
      ...
      expect(parsed.files.some((f: string) => f.endsWith("test1.ts"))).toBe(
        true
      );
      expect(parsed.files.some((f: string) => f.endsWith("test2.ts"))).toBe(
        true
      );
      expect(parsed.files.some((f: string) => f.endsWith("test3.js"))).toBe(
        false
      );
    ...
    it("should support multiple patterns", async () => {
      ...
      expect(parsed.files.some((f: string) => f.endsWith("app.jsx"))).toBe(
        true
      );
      expect(parsed.files.some((f: string) => f.endsWith("package.json"))).toBe(
        false
      );
  ...
  describe("Performance and Memory", () => {
    it("should not leak memory with repeated operations", async () => {
      ...
      for (let i = 0; i < 100; i++) {
        await fileReadTool.invoke(
          { filePath: testFile },
          { context: testContext }
        );
        await fileListTool.invoke(
          { directoryPath: testDir },
          { context: testContext }
        );
      }
