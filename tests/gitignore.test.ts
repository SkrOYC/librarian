...
describe("GitIgnoreService", () => {
  ...
  it("should handle negation patterns", async () => {
    ...
    expect(service.shouldIgnore(path.join(testDir, "important.txt"))).toBe(
      false
    );
  });
  ...
  it("should handle multiple negation patterns", async () => {
    ...
    expect(service.shouldIgnore(path.join(testDir, "important.log"))).toBe(
      false
    );
  ...
  it("should handle directory patterns", async () => {
    ...
    expect(service.shouldIgnore(path.join(testDir, "dist/bundle.js"))).toBe(
      true
    );
  ...
  it("should handle patterns with wildcards in subdirectories", async () => {
    ...
    expect(service.shouldIgnore(path.join(testDir, "node_modules"), true)).toBe(
      true
    );
    expect(
      service.shouldIgnore(
        path.join(testDir, "packages/app/node_modules"),
        true
      )
    ).toBe(true);
    expect(service.shouldIgnore(path.join(testDir, "src/index.js"))).toBe(
      false
    );
  });
});
