...
describe("Config Schema Alignment", () => {
  ...
  it("should support nested technology groups with branch and description", async () => {
    ...
    expect(loaded.technologies.default.react.repo).toBe(
  ...
  it("should auto-create default config when file missing", async () => {
    const nonExistentPath = path.join(
      process.cwd(),
      `test-auto-config-${Date.now()}.yaml`
    );
    ...
    try {
      ...
      expect(config.repos_path).toBe(
        path.join(os.homedir(), ".local/share/librarian/repos")
      );
    } finally {
  ...
  it("should create config directory if missing", async () => {
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        "LIBRARIAN_API_KEY=test-api-key"
      );
